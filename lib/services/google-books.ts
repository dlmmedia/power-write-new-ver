...
export class GoogleBooksService {
  ...
  async searchBestsellers(genre?: string): Promise<BookResult[]> {
    ...
    // Improved queries that work better with Google Books API
    const queries = genre
      ? [
          { query: `subject:${genre}`, maxResults: 20, startIndex: 0 },
          { query: `subject:${genre}`, maxResults: 20, startIndex: 20 },
        ]
      : [
          { query: `fiction`, maxResults: 20, startIndex: 0 },
          { query: `nonfiction`, maxResults: 20, startIndex: 0 },
  ...
  async searchNewReleases(genre?: string): Promise<BookResult[]> {
    ...
    const queries = genre
      ? [
          { query: `subject:${genre}+inpublisher:${currentYear}`, maxResults: 20, startIndex: 0 },
          { query: `subject:${genre}+inpublisher:${lastYear}`, maxResults: 20, startIndex: 0 },
        ]
      : [
          { query: `inpublisher:${currentYear}+fiction`, maxResults: 20, startIndex: 0 },
          { query: `inpublisher:${lastYear}+nonfiction`, maxResults: 20, startIndex: 0 },
        ];
    ...
    try {
      ...
      const queryPromises = queries.map(({ query, maxResults, startIndex }) => 
        this.searchBooks(query, maxResults, startIndex)
      );
  ...
  async searchByGenre(genre: string): Promise<BookResult[]> {
    ...
    try {
      ...
      console.log(`Found ${uniqueBooks.length} unique books for genre: ${genre}`);
