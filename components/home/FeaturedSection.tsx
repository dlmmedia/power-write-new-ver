...
export function FeaturedSection({ books, onSelectBook, isBookSelected }: FeaturedSectionProps) {
  ...
  useEffect(() => {
    if (currentIndex >= featuredBooks.length && featuredBooks.length > 0) {
      setCurrentIndex(0);
    }
            ...
            <div className="relative">
              ...
              {coverImageUrl ? (
                <img
                  ...
                  alt={currentBook.title}
                  className="relative w-[280px] h-[420px] object-contain rounded-lg shadow-2xl shadow-black/50 transform hover:scale-105 transition-transform duration-300"
                />
              ) : null}
            </div>
          </div>
