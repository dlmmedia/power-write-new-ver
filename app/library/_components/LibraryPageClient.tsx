...
import type { SeriesListItem } from '@/lib/types/series';
...
const BookCardItem = memo(function BookCardItem({
  ...
  onBookClick?: () => void;
  onAddToSeries?: (book: any) => void;
  onPromoteToSeries?: (book: any) => void;
}) {
      ...
      <Card variant="interactive" padding="none" className="overflow-hidden h-full">
        ...
        <div className="relative w-full aspect-[2/3] bg-[var(--background-tertiary)] overflow-hidden">
          ...
          <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
            ...
            {book.seriesId && (
              <Badge variant="info" style="soft" size="sm" className="gap-1">
                <BookCopy className="w-3 h-3" />
                {book.seriesNumber ? `#${book.seriesNumber}` : 'Series'}
              </Badge>
            )}
          </div>
          ...
          <div className="absolute top-2 left-2">
            <button
              ...
              className="p-1.5 rounded-md bg-black/40 text-white/90 hover:bg-black/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            {menuOpen && (
              <>
                ...
                />
                <div className="absolute left-0 mt-1 z-20 min-w-[180px] rounded-md border border-[var(--border)] bg-[var(--background-secondary)] shadow-lg py-1 text-sm">
                  {book.seriesId ? (
                    <button
                      type="button"
                      onClick={(e) => {
                      ...
                      className="w-full text-left px-3 py-1.5 text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] flex items-center gap-2"
                    >
                    ...
                    <>
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          e.stopPropagation();
                          setMenuOpen(false);
                          onPromoteToSeries?.(book);
                        }}
                        className="w-full text-left px-3 py-1.5 text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] flex items-center gap-2"
                      >
                        <Plus className="w-3.5 h-3.5" />
                        Promote to new series...
                      </button>
                    </>
                  )}
                </div>
...
function LibraryPageContent() {
  ...
  const [seriesActionBook, setSeriesActionBook] = useState<any | null>(null);
  const [seriesActionMode, setSeriesActionMode] = useState<'attach' | 'promote' | null>(null);
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--text-primary)]">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Link href="/library/series" className="hidden md:flex">
                <Button variant="ghost" size="sm" leftIcon={<BookCopy className="w-4 h-4" />}>Series</Button>
              </Link>
              <Link href="/library/series" className="md:hidden p-2 text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-hover)] rounded-lg transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredAndSortedBooks.map((book) => (
              <BookCardItem
                ...
                onAddToSeries={(b) => { setSeriesActionBook(b); setSeriesActionMode('attach'); }}
                onPromoteToSeries={(b) => { setSeriesActionBook(b); setSeriesActionMode('promote'); }}
              />
            ))}
          </div>
          </>
      ...
      <AttachToSeriesModal
        isOpen={seriesActionMode === 'attach' && !!seriesActionBook}
        book={seriesActionBook}
        onClose={() => { setSeriesActionBook(null); setSeriesActionMode(null); }}
        onAttached={() => {
          setSeriesActionBook(null);
          setSeriesActionMode(null);
          refreshBooks();
        }}
      />

      <PromoteToSeriesModal
        isOpen={seriesActionMode === 'promote' && !!seriesActionBook}
        book={seriesActionBook}
        onClose={() => { setSeriesActionBook(null); setSeriesActionMode(null); }}
        onPromoted={() => {
          setSeriesActionBook(null);
...
/** Pick an existing series to attach this book to. */
function AttachToSeriesModal({
  isOpen,
  book,
  onClose,
  onAttached,
  ...
  const handleAttach = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/books/${book.id}/series`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ seriesId: selected }),
      });
      const body = await res.json();
      if (!res.ok || !body.success) throw new Error(body?.error || 'Failed to attach');
      onAttached();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to attach to series');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title={`Add "${book.title}" to a series`} size="md">
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
          ...
          <button
            ...
            className="w-full text-left text-sm text-[var(--accent)] hover:underline flex items-center gap-1"
          >
            <Plus className="w-4 h-4" /> Create new series instead
          </button>
        </div>
      ...
      <SeriesEditorModal
        ...
        seedFromConfig={book.config || null}
        onSaved={async (created) => {
          setShowCreate(false);
          try {
            await fetch(`/api/books/${book.id}/series`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
...
/** Promote a book to a brand-new series, seeded from this book. */
function PromoteToSeriesModal({
  isOpen,
  book,
  onClose,
  onPromoted,
  ...
  useEffect(() => {
    if (isOpen && book) {
      const seriesName = book?.config?.basicInfo?.series?.name || `${book.title} Series`;
      setName(seriesName);
      setDescription('');
      setError(null);
    }
  ...
  const handlePromote = async () => {
    ...
    try {
      ...
      const body = await res.json();
      if (!res.ok || !body.success) throw new Error(body?.error || 'Failed to promote');
      onPromoted();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to promote to series');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Promote "${book.title}" to a new series`}
      description="Creates a new series, seeds shared defaults (style, world, themes) from this book, and adds it as Book #1."
      size="md"
    >
